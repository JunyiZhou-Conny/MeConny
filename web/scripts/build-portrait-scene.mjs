...
const [referencePath, characterPath, stickersPath, outputPath, calibrationPath = fileURLToPath(new URL('./portrait-calibration.json', import.meta.url))] = process.argv.slice(2)
assert(referencePath && characterPath && stickersPath && outputPath,
  'Usage: node web/scripts/build-portrait-scene.mjs reference.glb character.glb stickers-directory output.glb [calibration.json]')
...
assert(!fs.existsSync(outputPath) && !fs.existsSync(`${outputPath}.report.json`), 'Output or report already exists; choose a new path')
...
const out = {
  ...
  scene: 0, scenes: [{ name: 'Conny portrait candidate', nodes: [],
    ...(calibration.mobileFocusFraming ? { extras: { mobileFocusFraming: calibration.mobileFocusFraming } } : {}),
  }],
...
assert(!character.json.skins?.length && !character.json.animations?.length, 'Candidate must be static')
...
assert(out.textures.every(t => (t.source ?? t.extensions?.EXT_texture_webp?.source) < out.images.length))
...
for (const [index, node] of character.json.nodes.entries()) {
  ...
  assert.deepEqual(Object.keys(attrs).sort(), ['NORMAL', 'POSITION', 'TEXCOORD_0'])
  {
    ...
    targets.set(`${node.name}:0`, target)
  }
...
for (const sticker of calibration.stickers) {
  const target = targets.get(sticker.target)
  assert(target, `Missing explicit target: ${sticker.target}`)
  assert(sticker.depth > 0 && sticker.depth <= sticker.size, `Invalid shallow depth: ${sticker.id}`)
  ...
  const geometry = new DecalGeometry(target, position, helper.rotation, new THREE.Vector3(sticker.size, sticker.size, sticker.depth))
  ...
  decalReport.push({ id: sticker.id, target: sticker.target, depth: sticker.depth, triangles: p.count / 3, imageSha256: sha(imageBytes) })
  geometry.dispose()
}
for (const target of targets.values()) target.geometry.dispose()
assert.equal(decalReport.length, calibration.stickers.length)
...
fs.writeFileSync(outputPath, glb, { flag: 'wx' })
...
const report = {
  ...
  fit: calibration.fit, characterTriangles: triangleCount, decals: decalReport,
  checks: { cameraExact: true, anchorsExact: true, bothAnimationBytesAndTargetsExact: true, staticSourceTransformsIdentity: true,
